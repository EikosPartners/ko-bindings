...
_jquery2.default.fn.splitter = function (args) {
    args = args || {};
    return this.each(function () {
        if ((0, _jquery2.default)(this).is(".splitter")) // already a splitter
            return;
        ...
        function startSplitMouse(evt) {
            ...
            (0, _jquery2.default)(document).bind("mousemove" + opts.eventNamespace, doSplitMouse).bind("mouseup" + opts.eventNamespace, endSplitMouse);
        }
        ...
        function endSplitMouse(evt) {
            ...
            (0, _jquery2.default)(document).unbind("mousemove" + opts.eventNamespace + " mouseup" + opts.eventNamespace);
        }
        function resplit(pos) {
            ...
            if (!_jquery2.default.browser.msie) panes.trigger("resize");
        }
        ...
        var splitter = (0, _jquery2.default)(this).css({ position: "relative" }).addClass(opts.splitterClass);
        var panes = (0, _jquery2.default)(">*", splitter[0]).addClass(opts.paneClass).css({
            position: "absolute", // positioned inside splitter container
            "z-index": "1", // splitbar is positioned above
            "-moz-outline-style": "none" // don't show dotted outline
        });
        var A = (0, _jquery2.default)(panes[0]),
            B = (0, _jquery2.default)(panes[1]); // A = left/top, B = right/bottom
        opts.dockPane = opts.dock && (/right|bottom/.test(opts.dock) ? B : A);

        // Focuser element, provides keyboard support; title is shown by Opera accessKeys
        var focuser = (0, _jquery2.default)('<a href="javascript:void(0)"></a>').attr({ accessKey: opts.accessKey, tabIndex: opts.tabIndex, title: opts.splitbarClass }).bind((_jquery2.default.browser.opera ? "click" : "focus") + opts.eventNamespace, function () {
        ...
        // Splitbar element
        var bar = (0, _jquery2.default)('<div></div>').insertAfter(A).addClass(opts.barClass).addClass(opts.barStateClass).append(focuser).attr({ unselectable: "on" }).css({ position: "absolute", "user-select": "none", "-webkit-user-select": "none",
            "-khtml-user-select": "none", "-moz-user-select": "none", "z-index": "100" }).bind("mousedown" + opts.eventNamespace, startSplitMouse).bind("mouseover" + opts.eventNamespace, function () {
            (0, _jquery2.default)(this).addClass(opts.barHoverClass);
        }).bind("mouseout" + opts.eventNamespace, function () {
            (0, _jquery2.default)(this).removeClass(opts.barHoverClass);
        });
        ...
        A._pane = opts.side1;
        B._pane = opts.side2;
        _jquery2.default.each([A, B], function () {
            this._splitter_style = this.style;
            this._min = opts["min" + this._pane] || dimSum(this, "min-" + opts.split);
            this._max = opts["max" + this._pane] || dimSum(this, "max-" + opts.split) || 9999;
            this._init = opts["size" + this._pane] === true ? parseInt(_jquery2.default.curCSS(this[0], opts.split), 10) : opts["size" + this._pane];
        });
        ...
        if (opts.cookie) {
            if (!_jquery2.default.cookie) alert('jQuery.splitter(): jQuery cookie plugin required');
            initPos = parseInt(_jquery2.default.cookie(opts.cookie), 10);
            (0, _jquery2.default)(window).bind("unload" + opts.eventNamespace, function () {
                var state = String(bar.css(opts.origin)); // current location of splitbar
                _jquery2.default.cookie(opts.cookie, state, { expires: opts.cookieExpires || 365,
        ...
        if (opts.resizeTo) {
            ...
            (0, _jquery2.default)(window).bind("resize" + opts.eventNamespace, function () {
                var top = splitter.offset().top;
                var eh = (0, _jquery2.default)(opts.resizeTo).height();
                splitter.css("height", Math.max(eh - top - splitter._hadjust, splitter._hmin) + "px");
                if (!_jquery2.default.browser.msie) splitter.trigger("resize");
            }).trigger("resize" + opts.eventNamespace);
        } else if (opts.resizeToWidth && !_jquery2.default.browser.msie) {
            (0, _jquery2.default)(window).bind("resize" + opts.eventNamespace, function () {
        ...
        if (opts.dock) {
            ...
            if (opts.dockKey) (0, _jquery2.default)('<a title="' + opts.splitbarClass + ' toggle dock" href="javascript:void(0)"></a>').attr({ accessKey: opts.dockKey, tabIndex: -1 }).appendTo(bar).bind(_jquery2.default.browser.opera ? "click" : "focus", function () {
        ...
        splitter.bind("destroy" + opts.eventNamespace, function () {
            (0, _jquery2.default)([window, document]).unbind(opts.eventNamespace);
            bar.unbind().remove();
